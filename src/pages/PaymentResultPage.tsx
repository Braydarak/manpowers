...
function PaymentResultPage() {
  ...
  const displayOrderId = (() => {
    try {
      ...
      if (savedOrderId && savedOrderId.trim() !== '') return savedOrderId.trim();
    } catch (err) {
      ...
      if (lastResendAt) {
        ...
        if (!Number.isNaN(last)) {
          ...
          if (elapsedSec < windowSec) {
            setResendMsg(t('resend.throttle', { seconds: windowSec - elapsedSec }));
            setResending(false);
            return;
          }
        }
        ...
        <div className="max-w-4xl mx-auto px-4 md:px-8 lg:px-12 py-16">
          {paymentSuccess ? (
            ...
            <div className="bg-gray-800 border border-red-500 rounded-lg p-8 shadow-2xl text-center">
              ...
              <h2 className="font-bold text-3xl mb-4 text-white">{t('payment.error.title')}</h2>
              <p className="text-gray-300 mb-6">{t('payment.error.description')}</p>
              <div className="bg-gray-900 rounded-lg p-4 text-left inline-block max-w-full overflow-x-auto">
                {decodedMessage && <p className="text-gray-400"><strong className="text-white">{t('payment.error.messageLabel')}</strong> {decodedMessage}</p>}
                {displayOrderId && (
                  <p className="text-gray-400">
                    <strong className="text-white">{t('payment.error.orderLabel')}</strong> {displayOrderId}
                  </p>
                )}
              </div>
            </div>
