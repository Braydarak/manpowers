...
const Header: React.FC = () => {
  ...
  return (
    <header className="bg-gradient-to-b from-gray-900 to-black text-white py-4 px-6 w-full border-b border-gray-700 shadow-lg fixed top-0 z-50">
      <div className="max-w-7xl mx-auto flex justify-between items-center h-full">
        ...
        <h1 className="cursor-pointer" onClick={handleLogoClick}>
          <img 
            ...
            className="h-16 md:h-20 drop-shadow-[0_0_8px_rgba(255,255,255,0.3)] transition-transform duration-300 hover:scale-105"
          />
        </h1>
        ...
        <button 
          ...
          aria-label="Menú"
        >
          <span className={`block w-6 h-0.5 bg-white mb-1.5 transition-transform duration-300 ${menuOpen ? 'rotate-45 translate-y-2' : ''}`}></span>
        ...
        <div className="hidden md:flex flex-row items-center space-x-6">
          ...
          <div className="flex items-center space-x-2 ml-0 md:ml-6">
            <button 
              ...
              className={`px-2 py-1 rounded transition-all duration-300 cursor-pointer ${language === 'es' ? 'bg-white text-black shadow-[0_0_10px_rgba(255,255,255,0.5)]' : 'bg-gray-800 hover:bg-gray-700'}`}
            >
              ES
            </button>
            <button 
              onClick={() => handleLanguageChange('en')} 
              className={`px-2 py-1 rounded transition-all duration-300 cursor-pointer ${language === 'en' ? 'bg-white text-black shadow-[0_0_10px_rgba(255,255,255,0.5)]' : 'bg-gray-800 hover:bg-gray-700'}`}
            >
              EN
            </button>
          </div>
      ...
      <div 
        className={`fixed inset-0 bg-gradient-to-b from-gray-900 to-black z-40 transition-transform duration-300 ease-in-out ${menuOpen ? 'translate-x-0' : 'translate-x-full'} md:hidden flex flex-col justify-center items-center pt-20 pb-10 px-6`}
      >
        ...
        <div className="flex flex-col items-center space-y-8 w-full max-w-sm">
          ...
          <div className="flex items-center space-x-4 mt-6">
            <button 
              onClick={() => handleLanguageChange('es')} 
              className={`px-4 py-2 rounded-lg text-lg transition-all duration-300 ${language === 'es' ? 'bg-white text-black shadow-[0_0_10px_rgba(255,255,255,0.5)]' : 'bg-gray-800 hover:bg-gray-700'}`}
            >
              ES
            </button>
            <button 
              onClick={() => handleLanguageChange('en')} 
              className={`px-4 py-2 rounded-lg text-lg transition-all duration-300 ${language === 'en' ? 'bg-white text-black shadow-[0_0_10px_rgba(255,255,255,0.5)]' : 'bg-gray-800 hover:bg-gray-700'}`}
            >
              EN
            </button>
          </div>
