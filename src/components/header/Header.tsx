...
const Header: React.FC = () => {
  ...
  const showHomeInfoStripe = location.pathname === "/";
  ...
  return (
    ...
    >
      <header
        className={`bg-[var(--color-primary)] text-black py-1.5 md:py-2 px-4 md:px-6 w-full shadow-lg ${
          showHomeInfoStripe ? "" : "border-b border-black/10"
        }`}
      >
      ...
      {showHomeInfoStripe && (
        <>
          <div className="w-full bg-[var(--color-primary)] flex justify-center">
            <div className="h-px bg-black w-[70%]" />
          </div>
          <InfoStripe variant="freeShippingOver30" />
        </>
      )}
    </div>
  );
